...
api.listen(9090, async () => {
    ...
    console.log(chalk.blueBright(`===> Running: expo build:ios -t archive ${expoArguments}`));

    expoCli = cp.spawn('script', ['-r', '-q', '/dev/null', `expo build:ios -t archive ${expoArguments}`], {
        env: {
            ...process.env,
